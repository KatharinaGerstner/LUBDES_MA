...
#########################################################################################################################################################
LMM.MA.fit <- function(yi,vi,mods,slab,inner,outer){
  ##########################################################################################################################
...
#  VCov_matrix <- 
  rma.mv.fit <- rma.mv(yi=yi, V=vi, mods = mods, random = ~inner|outer, struct="CS", data=ES.frame, slab=slab,method="REML", tdist=FALSE, level=95, digits=4, Rscale="cor", sparse=FALSE, verbose=FALSE)
  return(rma.mv.fit)
}
...
mods <- 1
attach(ES.frame)
Richness.MA.fit <- LMM.MA.fit(yi=Richness.Log.RR,vi=Richness.Log.RR.Var,mods=~1,slab=paste(Study.Case,Low.LUI,High.LUI,sep="_"),inner=Study.Case,outer=Study.ID)
