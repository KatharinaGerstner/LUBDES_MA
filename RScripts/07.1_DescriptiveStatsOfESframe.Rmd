...
ggsave(path2temp %+% "biome_map.png", width=18, height=10, units="cm")
```
...
LUhistory_map <- ggplot(data=df, aes(y=Latitude, x=Longitude)) +
     geom_polygon(data = biome_mapdata, aes(x = long, y = lat, group = group, fill = NA)) +
...
![](c:\Users\hoppek\Documents\temp\Forest_plot_low-low.png)
![](c:\Users\hoppek\Documents\temp\Forest_plot_low-medium.png)
![](c:\Users\hoppek\Documents\temp\Forest_plot_low-high.png)
![](c:\Users\hoppek\Documents\temp\Forest_plot_medium-medium.png)
![](c:\Users\hoppek\Documents\temp\Forest_plot_medium-high.png)
![](c:\Users\hoppek\Documents\temp\Forest_plot_high-high.png)

```{r confounding variables}
library(gridExtra)
### Are BD and ES measured from the same species (e.g. weed BD and weed biomass in grasslands is linked)?
p1 <- ggplot(data=ES.frame.richness,aes(y=Log.RR,x=factor(ES.and.BD))) + 
  geom_boxplot() +
...
t.test(Log.RR ~ factor(ES.and.BD), data=ES.frame.richness)
...
t.test(Log.RR ~ factor(LU.definition.and.ES), data=ES.frame.yield)
...
plot(Log.RR~log10(Richness.Plot.Size),data=ES.frame.richness)
abline(fm,col="red") 
...
ggplot(data=ES.frame.yield,aes(y=Log.RR,x=factor(Yield.Unit.Type))) +
  geom_boxplot()+
  theme(axis.text = element_text(size = rel(2)), 
      axis.ticks = element_line(colour = "black"), 
      axis.title = element_text(size = rel(2)), 
      axis.text = element_text(size = rel(2))) +
  xlab("Yield.Unit.Type")
