...
for(i in c("LUI.range.level","Species.Group","Product","BIOME", "landuse_history")){
  p <- ggplot(data=ES.frame.richness) + 
...
LUhistory_map <- ggplot(data=df, aes(y=Latitude, x=Longitude)) +
     ...
     scale_fill_manual(values=c('#ffffb2','#fecc5c','#fd8d3c','#f03b20','#bd0026'), 
                       na.value="lightgrey",
                       name="",
                       limits=c("-5950","-50", "1450","1950", "11000"), 
                       labels=c("6000 BC","0", "1450","1950", "after 1950")) + 
  theme_lubdes(legend.position="bottom")

ggsave(path2temp %+% "LUhistory_map.png", width=22, height=10, units="cm")
# 
