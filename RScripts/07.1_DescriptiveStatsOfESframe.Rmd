...
ggsave(path2temp %+% "LUhistory_map.png", width=18, height=10, units="cm")
# 
...
ggplot(data=ES.frame.richness,aes(y=Log.RR,x=factor(ES.and.BD))) + 
  geom_boxplot()

t.test(Log.RR ~ factor(ES.and.BD), data=ES.frame.richness)

### Is land use intensity measured from data on yield.
ggplot(data=ES.frame.yield,aes(y=Log.RR,x=factor(LU.definition.and.ES))) + 
  geom_boxplot()
t.test(Log.RR ~ factor(LU.definition.and.ES), data=ES.frame.yield)

# ### Relationship spatial scale - log.RR
# 
# # 1. standardize units cf. function convertAreaUnits() in 01_load:libraries_and_functions.r (currently outcommented)
# table(data$reported.area.of.BD)
# table(data$reported.area.unit.of.BD)
# 
# # 2. add to ES.frame.richness
# 
# # 3. fit linear model
# fm <- lm(Log.RR ~ plot.size, data=ES.frame.richness)
# 
# # 4. plot data and fitted line
# ggplot(data=ES.frame.richness,aes(y=Log.RR,x=factor(ES.and.BD))) + 
#   geom_points() + geom_line()
