...
imp = mice(data[,c(31:32,35)],m=,method="rf",seed=42)
data[,c(31:32,35)] = complete(imp)

imp = mice(data[,c(41:42,45)],m=,method="rf",seed=42)
...
names(ES.frame) <- c("Study.ID","Case.ID","Low.LUI","High.LUI","Study.Type","Duration","Habitat.Type",        
                       "PES.Category","ES.From.BD","Fertilization","Irrigation","Pesticides",          
                       "Grazing","Mowing","Clear.Cut","Selective.Logging","Partial.Logging",     
                       "Additional.Treatment","Latitude","Longitude",           
...
table.sort = function(dat.low,dat.high,low,high){
  data.frame("Study.ID"=dat.low$Study.ID, "Case.ID" =dat.low$Case.ID, 
             ...
             "Duration" = dat.low$Date.of.study..end-dat.low$Date.of.study..start,
...
ES.frame$LUI.comparison <- paste(ES.frame$Low.LUI,ES.frame$High.LUI,sep="-")
ES.frame$LUI.range <- ES.frame$LUI.comparison
...
write.csv(ES.frame, "ES_table.csv")
