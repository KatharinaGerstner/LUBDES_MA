...
sign.terms <- Richness.MA.model[["select"]]$stats$terms[Richness.MA.model[["select"]]$stats$P<0.05]
print(sign.terms)
...
### Define predFrame for all possible combinations of one varying covariable m and all other fixed
sign.terms <- Yield.MA.model[["select"]]$stats$terms[Yield.MA.model[["select"]]$stats$P<0.05]
print(sign.terms)
comb.terms <- levels(factor(paste(modelDataYield[,paste(sign.terms[1])],modelDataYield[,paste(sign.terms[2])],sep="/")))
...
predsY <- predict.rma(Yield.MA.model[["select"]]$model,newmods = newMods)
predFrameY <- cbind(predFrameY,predsY=predsY$pred,predsY.se=predsY$se)

predFrame <- join(predFrameR,predFrameY,type="full")

## TO DO: Plot Crosses for all LUI comparisons on one plot simultaneously
for (lui in levels(predFrameR$LUI.range.level)){
  ...
  plot.lui <- ggplot(data=predFrame.subset) +
    geom_pointrange(aes(x=predsY, y=predsR, ymin=predsR - (1.96*predsR.se), ymax=predsR + (1.96*predsR.se)), color="black", size=1.5) +
    geom_segment(aes(x=predsY - (1.96*predsY.se), xend=predsY + (1.96*predsY.se), y = predsR, yend = predsR), color="black",size=1.5) +
...
#    scale_colour_brewer(palette="Set1",labels=paste(predFrame.subset$Class)) +
    ylab("RR (Species Richness)") + xlab("RR (Yield)") + labs("") + #labs(color=paste(lui)) +
  ...
  ggsave(plot.lui, file = paste(path2temp,"/New_Cross_diagram_",lui,".png",sep=""), width = 20, height = 8, type = "cairo-png")
}
# plot <- ggplot() + 
